...
import { removeFromCart, updateQtyCart } from "../../redux/actions/cartAction";

function Cart({ item, setLoading }) {
  ...
  const updateCartQtyInDB = (qty) => {
    if(qty<1) return;
    setQuantity(qty);
    dispatch(updateQtyCart({ id: item.id, qty, auth }));
  };

  const removeCartItem = async () => {
    setLoading(true);
    await dispatch(removeFromCart({ id: item.id, auth }));
    setLoading(false);
  };
    ...
    <div className="row mx-1 relative mb-4">
          ...
          <div className="item-img">
            <a href="/products/pc-gvn-x-msi-project-zero-white">
              <img
                ...
                alt="PC GVN x MSI PROJECT ZERO WHITE (Intel i5-13400F/ VGA RTX 4060)"
              />
            </a>
          </div>
          <div className="item-remove">
            <div className="remove">
              <button onClick={() => removeCartItem()} className="cart text-xs">
                <svg
                  width="16"
                  height="16"
                  viewBox="0 0 17 16"
      ...
      <div className="col col-7">
        <div className="item-info text-black">
          <a
            href="/products/pc-gvn-x-msi-project-zero-white"
          ...
          <div className="gifts-list text-xs">
            ...
            <div
              className="line-gift"
              data-line="2"
              data-variant-id="1115984334"
              data-pro-id="1051514299"
            >
              <div className="gift-info">
                Tặng:{" "}
                <a
                  className="text-black hover:text-black"
                  href="/products/ram-corsair-vengeance-rgb-32gb-2x16gb-5200-ddr5-white-cmh32gx5m2b5200c40w"
                >
                  Ram Corsair Vengeance RGB 32GB (2x16GB) 5200 DDR5 White
                  (CMH32GX5M2B5200C40W)
                </a>
      ...
      <div className="col col-3 text-right pe-1">
        <div className="item-price mb-2">
          <span className="text font-semibold">
            {formatNumberWithCommas(item?.product.salePrice)}₫
          </span>
        </div>
        <div className="item-quan text-sm">
          <div className="flex justify-end">
            <button
              type="button"
              className="p-1 border rounded-bl rounded-tl"
              onClick={() => quantity > 1 && updateCartQtyInDB(quantity - 1)}
            >
              <svg
                width="14"
                height="14"
            ...
            <button
              type="button"
              className="p-1 border rounded-br rounded-tr"
              onClick={() => {
                updateCartQtyInDB(quantity + 1);
              }}
            >
              <svg
                width="14"
