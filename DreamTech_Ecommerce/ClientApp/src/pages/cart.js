...
import { postDataAPIWithAuth } from "../utils/fetchData";
  ...
  const makeOrder = async (order) => {
    try {
      const res = await postDataAPIWithAuth(
        `Order/CustomerMakeOrder/${auth.user.id}`,
        order,
        auth.token
      );
      if (res) {
      ...
      <div className="cart-wrapper">
        ...
        <div className="cart-main rounded-sm mb-3 p-3">
          ...
          <div className="items-container mt-4">
            ...
            {items.length < 1 && (
              <div className="flex justify-center">
                <img
                  className
                  src="https://phongkhamdongxuan.com/assets/images/no-cart.png"
                />
              </div>
            )}
            {step === 2 && (
          ...
          <div className="summary py-3">
            ...
            {items.length > 0 && (
              <div
                className={`total-amount flex justify-between items-center font-semibold pt-1  ${
                  step === 4 && "d-none"
                }`}
              >
          ...
          {items.length > 0 && (
            <div className="action-section">
              <button
                className={`block text-center action-button uppercase w-full ${
                  step === 4 && "d-none"
                }`}
