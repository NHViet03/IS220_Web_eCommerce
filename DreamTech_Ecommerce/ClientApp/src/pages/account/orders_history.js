...
function OrdersHistory() {
  ...
  const filteredOrders = orderItems.filter(order => (
    ...
    (selectedStatus === 'all' || order.orderStatus.toString() === selectedStatus)
  ));
                  ...
                  <div class="box-info-account pt-4 block">
                    <div class="history-status block">
                      <div class="status-list items-center justify-between flex">
                        <a
                          ...
                          className={`status-item status-item px-1 pt-2 w-auto text-center relative pb-2 ${selectedStatus === 'all' ? 'text-red-600' : ''}`}
                          data-stt="all"
                          onClick={() => setSelectedStatus('all')}
                        >
                          <span
                            class="text font-semibold inline-block text-center ml-8 pb-2">TẤT CẢ</span><span class="count"></span>
                        </a>
                        <a
                          href="javascript:void(0);"
                          className={`status-item status-item px-1 pt-2 w-auto text-center relative pb-2 ${selectedStatus === '0' ? 'text-red-600' : ''}`}
                          data-stt="processing"
                        ...
                        <a
                          href="javascript:void(0);"
                          className={`status-item status-item px-1 pt-2 w-auto text-center relative pb-2 ${selectedStatus === '2' ? 'text-red-600' : ''}`}
                          data-stt="delivering"
                          onClick={() => setSelectedStatus('2')}
                        >
                          <span class="text font-semibold inline-block text-center">ĐÃ VẬN CHUYỂN</span><span class="count d-none"></span>
                        </a>
                        <a
                          href="javascript:void(0);"
                          className={`status-item status-item px-1 pt-2 w-auto text-center relative pb-2 ${selectedStatus === '1' ? 'text-red-600' : ''}`}
                          data-stt="cancel"
                          onClick={() => setSelectedStatus('1')}
                        >
                          <span class="text font-semibold inline-block text-center mr-8">ĐÃ HỦY</span><span class="count d-none"></span>
                        </a>
                      </div>
                    </div>
                    <div class="history-search py-2 bgcolor-app-gray block">
                      <div class="box-history-search relative flex justify-between box-border bg-white rounded border">
                        ...
                        <button className='px-4 text-base whitespace-nowrap font-semibold color-app-blue-nd' id="btn-history-search">Tìm đơn hàng</button>
                      </div>
                    </div>
                    <div className="history-table bg-white" data-table="all">
                      {filteredOrders.length > 0 ? (
                        filteredOrders.map((order) => (
                          <div key={order.id} className="history-table__item" data-search={`#${order.id}`} data-stt="new">
                            ...
                            <div className="history-table__item-body">
                              {order.orderDetails.map((orderDetail) => (
                                <div key={orderDetail.id} className="history-table__item-line">
                                  <div className="left">
                                    <div className="img-line">
                                      <img src={orderDetail.product.productImages[0].imageUrl} alt="Product Image" />
                                      <div className="qty-line">x{orderDetail.qty}</div>
                                    </div>
                                    <div className="info-line">
                                      <div className="name-line">{orderDetail.product.name}</div>
                                    </div>
                                  </div>
                                  <div className="right">
                                    <div className="text-right">
                                      <div className="price-line">{formatPrice(orderDetail.product.price)}</div>
                                    </div>
                                  </div>
                                </div>
                              ))}
                            ...
                            <div className="history-table__item-foot">
                              <div className="total-order text-right">
                                <span className='font-medium text-emerald-500'>{order.payments.status}</span><br />
                                <span>Hình thức thanh toán: {order.payments.method}</span><br />
                                <span>Tổng tiền: {formatPrice(order.totalAmount)}</span>
                              </div>
                        ...
                        <div class="data-account__empty text-center w-full mt-8 mb-6 box-border block">
                          ...
                          <p class="alert-empty text-base my-4 font-semibold mb-4">Quý khách không có đơn hàng.</p>
                          <NavLink to="/">
                            <button className="button relative px-7 py-2 font-semibold text-white bgcolor-app-primary rounded">TIẾP TỤC MUA HÀNG</button>
                          </NavLink>
                        </div>
                      )}
                    </div>
